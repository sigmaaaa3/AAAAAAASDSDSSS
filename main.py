...
ADMIN_ID = int(os.getenv("ADMIN_ID", "1481795711"))

if not BOT_TOKEN:
    logging.error("❌ BOT_TOKEN не знайдено в змінних оточення")
    exit()
...
def download_model():
    """Завантажує модель з Hugging Face Hub."""
    from transformers import AutoTokenizer, TFAutoModelForSequenceClassification
    import os
    
    model_name = "Fgsjfgfqdf/iiiiassdasgadssdfjqd"  # Заміни на свою модель
    cache_dir = "./model_cache"
    
    logger.info("Завантаження токенізатора та моделі AI з Hugging Face...")
    ...
    try:
        ...
        # Спроба завантажити локальну модель, якщо вона є
        try:
            tokenizer = AutoTokenizer.from_pretrained(".", local_files_only=True)
            model = TFAutoModelForSequenceClassification.from_pretrained(".", local_files_only=True)
            logger.info("✅ Модель AI завантажена локально.")
            return tokenizer, model
        except:
